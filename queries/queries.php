...
$terminal = "";
$box = "";
$result = null; // começa vazio
$nome_pt = "";  // começa vazio

// Só executa se tiver busca (ponto/terminal/box/serviço)
if (
    ...
    if (isset($_GET['buscar']) && !empty($_GET['buscar'])) {
        ...
        $filtro_sql .= " AND st.stop_code LIKE '%$pesquisa%'";
    }
    ...
    // Definir nome do ponto para o caption a partir da query principal
    $nome_pt = "";
    if ($result && mysqli_num_rows($result) > 0) {
        $row_first = mysqli_fetch_assoc($result);

        // Prioridade: box selecionado, depois código do ponto
        if (!empty($box)) {
            $nome_pt = $row_first['box'];
        } elseif (!empty($pesquisa)) {
            $nome_pt = $row_first['box'];
        } else {
            $nome_pt = $row_first['box']; // primeiro box retornado
        }

        // Reposiciona ponteiro para poder usar while
                        ...
                        <tbody>
                            <?php while ($sql_result = mysqli_fetch_array($result)): ?>
                                <tr>
                                    ...
                                    <td class="td-dest"><?php echo $sql_result['stop_headsign'] ?></td>
